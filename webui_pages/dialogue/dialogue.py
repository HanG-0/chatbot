...
import server.db.database_manager as db
redis = get_redis()
...
def get_messages_history(chat_name:str, history_len: int, content_in_expander: bool = False) -> List[Dict]:
    '''
    返回消息历史。
    history_len：限制返回的历史数据数量
    content_in_expander控制是否返回expander元素中的内容，一般导出的时候可以选上，传入LLM的history不需要
    '''

    def filter(msg):
        content = [x for x in msg["elements"] if x._output_method in ["markdown", "text","richmd"]]
    ...
    return chat_box.filter_history(history_len=history_len, filter=filter,chat_name=chat_name)
...
def dialogue_page(api: ApiRequest, is_lite: bool = False):
    ...
    with st.sidebar:
        ...
        st.divider()
        st.subheader("对话状态")
        status_options = ["S0", "S1", "S2", "S3", "S4", "S5", "S6"]
        def on_status_change():
            new_status = st.session_state.status_selector
            redis.setex("status_store",3000,new_status)
            st.toast(f"状态已更新为: {new_status}")
        cur_status = redis.get("status_store")
        if cur_status is None:
            logger.info("dialog: redis中status_store为空，设置为默认值")
            cur_status = "S0"
    ...
    if st.session_state.run_once:
        if prompt := st.chat_input(chat_input_placeholder, key="prompt1"):
            if parse_command(text=prompt, modal=modal):  # 用户输入自定义命令
                ...
                st.info(f"当前状态: {cur_status}")
                history = get_messages_history(chat_box.cur_chat_name,history_len)
                if dialogue_mode == "LLM 对话":
                    ...
                    # 确保 message_id 不为空，否则使用唯一标识
                    feedback_key = message_id if message_id else uuid.uuid4().hex
    ...
    if prompt := st.chat_input(chat_input_placeholder, key="prompt"):
        ...
        if parse_command(text=prompt, modal=modal):  # 用户输入自定义命令
            ...
            if dialogue_mode == "自定义Agent问答":
                cur_status = redis.get("status_store")
                st.info(f"当前状态: {cur_status}")
                st.toast(f"工作流状态更新: {cur_status}")
                db.update_case_progress(st.session_state.username,cur_status)
