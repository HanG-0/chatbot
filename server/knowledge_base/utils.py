...
# 🔑 关键1：在替换前，保存原始函数对象的引用（不是模块属性！）
_ORIGIN_JSON_DUMPS = _json_module.dumps

def _new_json_dumps(obj, **kwargs):
    """安全的 json.dumps 包装：禁用 ensure_ascii + 处理函数对象"""
    # 🔑 关键2：复制 kwargs 避免副作用
    kwargs = kwargs.copy()
    kwargs["ensure_ascii"] = False

    # 🔑 关键3：添加安全的 default 处理器
    if 'default' not in kwargs:
        def _safe_default(o):
            if callable(o):  # 函数/lambda/方法
                return f"<function:{o.__name__}>" if hasattr(o, '__name__') else "<callable>"
            if hasattr(o, '__dict__'):  # 自定义类实例
                return o.__dict__
            if isinstance(o, set):  # 集合转列表
                return list(o)
            if isinstance(o, bytes):  # bytes 转字符串
                return o.decode('utf-8', errors='replace')
            # 其他无法处理的类型，交给原始逻辑抛出标准错误
            raise TypeError(f"Object of type {type(o).__name__} is not JSON serializable")
        kwargs['default'] = _safe_default

    # 🔑 关键4：调用保存的原始函数对象（不是模块属性！）
