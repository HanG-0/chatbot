...
class ApiModelWorker(BaseModelWorker):
    ...
    def generate_stream_gate(self, params: Dict):
        ...
        self.logger.info(f"[{time.time()}] generate_stream_gate called")
        self.logger.info(f"Worker: {self.worker_addr}")
        self.logger.info(f"Model: {self.model_names}")
        self.logger.info(f"Params keys: {params.keys()}")
        self.logger.info(f"Prompt length: {len(params.get('prompt', ''))}")
        self.logger.info(f"Call count: {self.call_ct}")
        try:
            self.logger.info(f"Active tasks: {len(asyncio.all_tasks())}")
        except RuntimeError:
            self.logger.info("Active tasks: (cannot get - no running event loop)")

        # 检查并发限制
        self.logger.info(f"Semaphore value: {self.semaphore._value}")
        try:
            self.logger.info("")
            prompt = params["prompt"]
    ...
    def prompt_to_messages(self, prompt: str) -> List[Dict]:
        ...
        ai_start = ai_role + ":"
        system_start = system_role + ":"
        self.logger.info(f'self.ai_role：{self.ai_role}, self.user_role：{self.user_role}')

        for msg in prompt.split(self.conv.sep)[:-1]:
            if msg.startswith(user_start):
                ...
                if content := msg[len(system_start):].strip():
                    result.append({"role": system_role, "content": content})
            else:
